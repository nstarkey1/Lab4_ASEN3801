...
for idx = 1:length(cases_25)
    ...
    figure(figs_25k(1));
    legend('Uncontrolled', 'Rate Feedback', 'Location', 'best');
    ...
    subplot(4,1,1); plot(t_unc, (m*g/4)*ones(size(t_unc)), 'b', 'LineWidth', 1.5); hold on;
    ...
    for mi = 1:4
        subplot(4,1,mi);
        plot(t_ctrl, mf_ctrl_all(mi,:), 'g', 'LineWidth', 1.5); hold on;
...
% This function just adds supertitles to each figure (I used gemini for
% this function ngl so it might be iffy) (future max dont forget to remove
% this comment)
...
%% Task 3.3 Linear Closed-Loop Simulations
plots33 = true;
cases_33 = {
    4,  deg2rad(5), '+5 deg roll';
    5,  deg2rad(5), '+5 deg pitch';
    10, 0.1,        '+0.1 rad/s roll rate';
    11, 0.1,        '+0.1 rad/s pitch rate';
};

for k = 1:length(cases_33)

    state_idx = cases_33{k,1};
    deviation = cases_33{k,2};
    label     = cases_33{k,3};
    ...
    ode_lin = @(t,var) QuadrotorEOM_Linearized_InnerLoop(t, var, g, m, I);
    [t33, X33] = ode45(ode_lin, t_span, IC_k);

    % Recompute control for plotting
    ctrl33 = zeros(4, length(t33));
    for ii = 1:length(t33)
        [Fc_ii, Gc_ii] = InnerLoopFeedback(X33(ii,:)');
        ctrl33(:,ii) = [Fc_ii(3); Gc_ii];
    end
    ...
    if plots33
        PlotAircraftSim(t33, X33', ctrl33, figs33, 'b');
        sgtitle_figs(figs33, sprintf('Task 3.3 Case %d: %s (Linear CL)', k, label));
    end
end

%% 3.4 Linear vs Nonlinear Closed-Loop Comparison
plots34 = true;
cases_34 = {
    4,  deg2rad(5), '+5 deg roll';
    5,  deg2rad(5), '+5 deg pitch';
...
for k = 1:length(cases_34)

    state_idx = cases_34{k,1};
    deviation = cases_34{k,2};
    label     = cases_34{k,3};
    ...
    ode_lin = @(t,var) QuadrotorEOM_Linearized_InnerLoop(t, var, g, m, I);
    [t_lin, X_lin] = ode45(ode_lin, t_span, IC_k);

    ctrl_lin = zeros(4, length(t_lin));
    for ii = 1:length(t_lin)
        [Fc_ii, Gc_ii] = InnerLoopFeedback(X_lin(ii,:)');
        ctrl_lin(:,ii) = [Fc_ii(3); Gc_ii];
    end

    % Nonlinear closed-loop simulation
    ode_nl = @(t,var) QuadrotorEOM_Nonlinear_InnerLoop(t, var, g, m, I, d, km, nu, mu);
    [t_nl, X_nl] = ode45(ode_nl, t_span, IC_k);

    ctrl_nl = zeros(4, length(t_nl));
    for ii = 1:length(t_nl)
        [Fc_ii, Gc_ii] = InnerLoopFeedback(X_nl(ii,:)');
        ctrl_lin(:,ii) = [Fc_ii(3); Gc_ii(:)];
        ctrl_nl(:,ii)  = [Fc_ii(3); Gc_ii(:)];
    end
    ...
    if plots34
        ...
        size(X_lin)
        length(t_lin)
        PlotAircraftSim(t_lin, X_lin', ctrl_lin, figs34, 'r');
        size(ctrl_nl)
        size(X_nl)
        length(t_nl)
        PlotAircraftSim(t_nl, X_nl', ctrl_nl, figs34, 'b');
        sgtitle_figs(figs34, sprintf('Task 3.4 Case %d: %s', k, label));

        figure(figs34(1));
        legend('Linear CL', 'Nonlinear CL', 'Location', 'best', 'AutoUpdate', 'off');
    end
end